        ...
        for(int i = 0; i < NPDENTRIES; i++)
        {
          if(pgdir[i] & PTE_P)
          {
            ...
            for(int j = 0; j < NPTENTRIES; j++)
            {
              ...
              {
                int block_no = PTE_ADDR(pte[j]) >> PTXSHIFT;
...
void unset_access_bit(struct proc* p)
{
  // Unset the access bit of 10% pages
  // cprintf("\n\nHERE\n\n");
  pde_t* pgdir = p->pgdir;
  int count = 0;
  for(int i = 0; i < NPDENTRIES; i++)
  {
    if(pgdir[i] & PTE_P)
    {
      ...
      for(int j = 0; j < NPTENTRIES; j++)
      {
        if((pte[j] & PTE_P) && (pte[j] & PTE_U) && (pte[j] & PTE_A))
        {
          // cprintf("JIJIJI\n");
          if(count % 10 == 0)
          {
            ...
            {
              if (get_rmap_pid(PTE_ADDR(pte[j]), k) != -1)
              {
                struct proc *p;
                p = get_proc(get_rmap_pid(PTE_ADDR(pte[j]), k));
                pte_t *temp;
                temp = get_pte(p->pgdir, PTE_ADDR(pte[j]));
                *temp &= ~PTE_A;
              }
            }
          }
...
pte_t* victim_page(struct proc* p)
{
  pde_t* pgdir = p->pgdir;
  for(int i = 0; i < NPDENTRIES; i++)
  {
...
pte_t* page_to_swap(struct proc* p)
{
  pte_t* pte = victim_page(p);
  while(pte == 0)
  {
    // cprintf("Hello world!\n");
    unset_access_bit(p);
